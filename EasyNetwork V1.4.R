...
#especially when the matrix or data.frame is symmetric or triangle.
	...
	if(!is.matrix(x)){
		...
		else error("Input is not a dataframe or matrix.")
	}
...
to Col names.")
		...
		else error("You said it's a triangle matrix, but where is
 the data?")
	}
#to convert	
	c(x)->value
...
#value_p table and the very first column in the table should be 
